...
export const getOrderDetails = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!order) {
    return next(
      new ErrorHandler(`no order found with the order id :${req.params._id}`)
    );
  }
...
export const updateOrders = catchAsyncErrors(async (req, res, next) => {
  ...
  if (order?.orderStatus === "Delivered") {
    return next(
      new ErrorHandler(
        `Order with id: ${req?.params?.id} is already shipped`,
        400
      )
    );
  }
  ...
  order.orderItems.forEach(async (item) => {
    ...
    if (!product) {
      return next(
        new ErrorHandler(`Product not found with id:${item.product}`, 400)
      );
    }
